	...
	do
	{
		...
		if (cmd == "open")
		{
			...
			else
			{
				...
				if (playingField[xCoord][yCoord] == 0)
				{
					if (xCoord == 0) // isTop
					{
						if (yCoord == 0) // isLeft
						{
							maskedField[xCoord][yCoord + 1] = playingField[xCoord][yCoord + 1];
							maskedField[xCoord + 1][yCoord] = playingField[xCoord + 1][yCoord];
							maskedField[xCoord + 1][yCoord + 1] = playingField[xCoord + 1][yCoord + 1];
						}
						else if (yCoord == nNum) // isRight
						{
							maskedField[xCoord][yCoord - 1] = playingField[xCoord][yCoord - 1];
							maskedField[xCoord + 1][yCoord - 1] = playingField[xCoord + 1][yCoord - 1];
							maskedField[xCoord + 1][yCoord] = playingField[xCoord + 1][yCoord];
