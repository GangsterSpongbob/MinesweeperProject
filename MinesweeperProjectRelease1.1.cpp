...
bool checkInteger(string check)
{
	...
	while (isdigit(check[i++])) if (check[i] == '\0') return 1;
...
void openTile(int ** playingField, int ** maskedTable, int rowCol, int xCoord, int yCoord)
{
	if (maskedTable[xCoord][yCoord] == 10)
	{
		...
		else maskedTable[xCoord][yCoord] = playingField[xCoord][yCoord];
	}
}

bool gameOver(int** mask, int rowCol)
{
	for (int i = 0; i < rowCol; i++) for (int j = 0; j < rowCol; j++) if (mask[i][j] == 9) return 1;
...
bool gameWon(int** mask, int** table, int rowCol)
{
	for (int i = 0; i < rowCol; i++)
	{
		...
		{
			if (mask[i][j] == 10) return 0;

			if (table[i][j] != 9 && mask[i][j] == 11) return 0;
...
void printTable(int** table, int rowCol, int minesLeft)
{
	...
	for (int i = 0; i < rowCol; i++)
	{
		...
		{
			cout << ' ';
			if (table[i][j] == 9) cout << "\033[31m" << 'M' << "\033[0m";  // 9 marks a mine
			else if (table[i][j] == 10)	cout << "\033[100m" << ' ' << "\033[0m";  // 10 marks undiscovered square
			else if (table[i][j] == 11) cout << "\033[31m" << '!' << "\033[0m"; // 11 marks marked square (marked mine)
...
int** maskTable(int rowCol)
{
	...
	for (int i = 0; i < rowCol; i++) tableMask[i] = new int[rowCol] {};

	for (int i = 0; i < rowCol; i++) for (int j = 0; j < rowCol; j++) tableMask[i][j] = 10;
...
int** populateTable(int rowCol, int mineCount)
{
	...
	if (rowCol < 3 || rowCol > 9) exit(-1);  // throws warning otherwise
		...
		for (int j = 0; j < rowCol; j++)
		{
			if (tableArr[i][j] != 9)
			{
				...
				else // isNeither
				{
					if (j == 0) //isLeft
					{
						tableArr[i][j] = (tableArr[i - 1][j] == 9) + (tableArr[i - 1][j + 1] == 9) + (tableArr[i][j + 1] == 9) + (tableArr[i + 1][j] == 9) + (tableArr[i + 1][j + 1] == 9);
					}
					else if (j == rowCol - 1) // isRight
					{
						tableArr[i][j] = (tableArr[i - 1][j - 1] == 9) + (tableArr[i - 1][j] == 9) + (tableArr[i][j - 1] == 9) + (tableArr[i + 1][j - 1] == 9) + (tableArr[i + 1][j] == 9);
					}
					else // isNeither
					{
...
int main()
{
	...
	do
	{
		...
		if (cmd == "open")
		{
			...
			{
				message = "";
				openTile(playingField, maskedField, rowColumnCount, xCoord, yCoord);
			}
		}
